...
![Figure 3: ACDC per-condition mIoU before and after Step 2 (full-data pipeline)](figures/fig_before_after_bar_full.png)

**Figure 3:** Per-condition ACDC validation mIoU before and after the Step 2 adaptation (full-data B2 pipeline, 24,966 GTA5 pairs). Night improves in this run (+1.74 pp), in contrast to the partial-data run where it regressed (−3.1 pp), confirming that stronger Step 1 representations resolve the night regression.

![Figure 4: Step 2 per-condition adaptation curve (full-data pipeline)](figures/fig_step2_adaptation_curve_full.png)

**Figure 4:** Per-condition mIoU over Step 2 training iterations for the full-data run. All four conditions improve monotonically through iteration 5,000 (best checkpoint), including night -- which flatlines near 8% in the partial run but climbs steadily here.
