...
The *before-step-2* evaluation applies the Step 1 checkpoint directly to the ACDC validation split without any Step 2 training. For the partial-data run this measurement was taken at iteration 50 of Step 2 (before any meaningful adaptation); for the full-data run it was performed via standalone evaluation of the Step 1 checkpoint prior to initiating Step 2 training. In both cases the result represents the unadapted baseline. The *after-step-2* evaluation uses the best Step 2 checkpoint, selected as the checkpoint achieving the highest ACDC val mIoU at any point during Step 2 training. This selection criterion is consistent across all experiments.
...
**Table 4: Full-data pipeline results (SegFormer-B2, 24,966 GTA5 pairs).**

| Stage | Overall mIoU | Fog | Rain | Snow | Night |
|---|---|---|---|---|---|
| Before Step 2 (full-data Step 1 on ACDC) | 32.07% | 40.54% | 35.89% | 33.38% | 15.02% |
| After Step 2 (full-data pipeline) | **38.18%** | **49.08%** | **42.71%** | **40.54%** | **16.77%** |
| Δ (Step 2 gain) | **+6.11 pp** | **+8.54 pp** | **+6.83 pp** | **+7.16 pp** | **+1.74 pp** |

*Note: "Before Step 2" for the full-data run was measured by applying the Step 1 best checkpoint (iter 24,000) directly to the ACDC validation split using standalone evaluation (no Step 2 training had occurred). "After Step 2" uses the best checkpoint from Step 2 training (iteration 5,000), selected by ACDC val mIoU.*

![Figure 2: ACDC per-condition mIoU before and after Step 2 (full-data pipeline)](figures/fig_before_after_bar_full.png)

**Figure 2:** Per-condition ACDC validation mIoU before and after the Step 2 adaptation (full-data B2 pipeline, 24,966 GTA5 pairs). Night improves in this run (+1.74 pp), in contrast to the partial-data run where it regressed (−3.1 pp), confirming that stronger Step 1 representations resolve the night regression.

![Figure 3: Step 2 per-condition adaptation curve (full-data pipeline)](figures/fig_step2_adaptation_curve_full.png)

**Figure 3:** Per-condition mIoU over Step 2 training iterations for the full-data run. All four conditions improve monotonically through iteration 5,000 (best checkpoint), including night -- which flatlines near 8% in the partial run but climbs steadily here.
...
![Figure 4: Step 2 per-condition adaptation curve (partial-data run)](figures/fig_step2_adaptation_curve.png)

**Figure 4:** Per-condition mIoU over Step 2 training iterations (top panel) and overall mIoU (bottom panel), for the partial-data run. Night drops immediately at iteration 1,000 and flatlines near 8% for the remainder of training, while fog, rain, and snow climb steadily to the best checkpoint at iteration 9,000.

**Full-data run: regression is not fundamental.** With the stronger Step 1 checkpoint (38.90% CS val mIoU) as initialization, night mIoU improves after Step 2: 15.02% → **16.77%** (+1.74 pp). Night no longer regresses  -  it gains, alongside every other condition. This confirms that the night regression observed in the partial run was not a fundamental limitation of pseudo-label UDA, but a consequence of the weak Step 1 model: a stronger intermediate representation generates more reliable night pseudo-labels, breaking the class-imbalance feedback loop that caused regression in the partial run.
...
**The two runs form a natural experiment.** The partial-data and full-data pipelines differ in exactly one variable: Step 1 dataset size, which directly controls Step 1 quality (30.30% vs 38.90% CS val mIoU). Everything else  -  backbone, training procedure, Step 2 config  -  is identical. The fact that night flips from regression (−3.1 pp) to stability or improvement across these two runs isolates Step 1 quality as the *cause* of night's behaviour, not merely a correlate. This is a stronger claim than a single run could support: it identifies the mechanism, not just the outcome, and points directly to where effort should be invested in future work.
...
For the partial-data B2 pipeline, Step 2 yields a net overall improvement of **+5.52 mIoU points** on ACDC (27.20% → 32.72%). The improvement is condition-specific: fog benefits most (+12.3 pp), rain and snow improve substantially (+9.4, +8.6 pp), and night uniquely regresses (−3.1 pp) under the weak partial-data Step 1 model. Critically, the full-data run confirms this regression is not fundamental to the approach: with a stronger Step 1 checkpoint (38.90% CS val mIoU), night improves by +1.74 pp (15.02% → 16.77%), indicating that better intermediate representations generate more reliable night pseudo-labels and break the class-imbalance feedback loop that caused regression in the partial run.

The full-data B2 pipeline  -  training Step 1 on the complete 24,966-pair GTA5 dataset  -  achieves **38.18% overall mIoU** after Step 2 (fog=49.08%, rain=42.71%, snow=40.54%, night=16.77%), a gain of **+6.11 pp** over the before-step-2 baseline of 32.07%. Every condition improves, including night (+1.74 pp), in contrast to the partial-data run where night regressed. The stronger Step 1 checkpoint (38.90% CS val mIoU, +8.60 pp over the partial Step 1) translates directly into a larger and more consistent Step 2 gain across all conditions.

The main scientific conclusion is clear even from the partial-data results: **sequential UDA improves ACDC segmentation, the gain is condition-specific, and the quality of the intermediate adaptation step is the primary bottleneck**. The two runs together function as a natural experiment  -  identical in every respect except Step 1 dataset size  -  and night's behaviour flipping from regression (−3.1 pp, partial) to improvement (+1.74 pp, full-data) isolates Step 1 quality as the causal mechanism. The night regression observed in the partial run is therefore a consequence of a weak Step 1 model, not a fundamental limitation of the approach, which reframes the design priority toward investing in Step 1 quality rather than night-specific pseudo-label interventions. Future work should investigate whether MIC-style masked image consistency provides further weather-invariant regularization in the Step 2 loop.
...
Yang, X., Yan, W., Yuan, Y., Mi, M. B., and Tan, R. T. (2024). Semantic segmentation in multiple adverse weather conditions with domain knowledge retention. *AAAI 2024*. arXiv:2401.07459.
